...
window.DeckChrome = (function () {
  ...
  const PARTS = [
    ...
    { id: 'gen',     label: 'Generators',  accent: '#22d3ee', accent2: '#60a5fa' },
    { id: 'analyze', label: 'Analyzers',   accent: '#fbbf24', accent2: '#fb923c' },
    { id: 'perf',    label: 'Performance', accent: '#fb7185', accent2: '#f472b6' },
    { id: 'skill',   label: 'The Skill',   accent: '#34d399', accent2: '#22d3ee' }
  ];
  ...
  function syncHint(current) {
    ...
    if (el.parentElement.matches('section') &&
        el.parentElement.querySelectorAll(':scope > section').length > 1) {
      // inside a stack: nothing extra
    }
    const stack = el.closest('.slides > section');
    if (stack && stack.querySelectorAll(':scope > section').length > 1 && current.v === 0) {
      bits.push('↓ more');
    } else if (stack === el && stack.querySelectorAll(':scope > section').length) {
      // no children
    }
